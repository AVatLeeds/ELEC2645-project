...
float FFT::recursive_DFT(uint16_t indicies[], unsigned int exponent, unsigned int f)
{
...
FFT::FFT()
{
    ...
    {
        _coefficients[f] = cos((2 * M_PI * static_cast<float>(f)) / static_cast<float>(max_FFT_length));
    }
    _coefficients[f] = 0.0;
}
