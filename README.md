...
The FFT algorithm is an specific optimisation of the Discrete Fourier Transform (DFT), the mathematical method for determining the frequency content of a discrete set of samples of a signal. A naive algorithmic implementation of the DFT, as it is defined, would have a computation time of $O(N^2)$, meaning the time it would take to complete is proportional to the square of the number of samples ($N$). The FFT has a computation time of $O(N \log_2 N )$, a vast improvement over the DFT, especially for large $N$. Here's how it works.
...
$$ S[f] = \sum_{n = 0}^{N-1} {s[n] (e^{-2 \pi j {n \over N}})^f} $$
...
If the samples are viewed on the complex plane, multiplying the value of each sample by $e^{-2 \pi j {n \over N}}$ rotates it anticlockwise through an angle that is equivalent to it's position in the sample sequence. PICTURE
...
$$ S[f] = \sum_{m = 0}^{{N \over 2}-1} {s[2m] (e^{-2 \pi j {2m \over N}})^f} + \sum_{m = 0}^{{N \over 2}-1} {s[2m + 1] (e^{-2 \pi j {2m + 1 \over N}})^f} $$
expanding the exponent of $e$ gives
$$ S[f] = \sum_{m = 0}^{{N \over 2}-1} {s[2m] (e^{-2 \pi j {2m \over N}})^f} + \sum_{m = 0}^{{N \over 2}-1} {s[2m + 1] (e^{-2 \pi j {2m \over N}}e^{-2 \pi j {{1 \over N}}})^f} $$
and multiplying $f$ into one of the exponents
$$ S[f] = \sum_{m = 0}^{{N \over 2}-1} {s[2m] (e^{-2 \pi j {2m \over N}})^f} + \sum_{m = 0}^{{N \over 2}-1} {s[2m + 1] (e^{-2 \pi j {2m \over N}})^f e^{-2 \pi j {{f \over N}}}} $$
...
$$ S[f] = \sum_{m = 0}^{{N \over 2}-1} {s[2m] (e^{-2 \pi j {2m \over N}})^f} + e^{-2 \pi j {{f \over N}}} \sum_{m = 0}^{{N \over 2}-1} {s[2m + 1] (e^{-2 \pi j {2m \over N}})^f} $$
...
$$ S[f] = DFT(2m) + e^{-2 \pi j {{f \over N}}} DFT(2m + 1) $$
...
$$ S[f] = \sum_{n = 0}^{N-1} {s[n] (e^{-2 \pi j {n \over N}})^f} = \sum_{n = 0}^{1} {s[n] (e^{-2 \pi j {n \over 2}})^f} = s[0](e^{-2 \pi j {0 \over 2}})^f + s[1](e^{-2 \pi j {1 \over 2}})^f = s[0] + s[1]e^{-\pi j f} $$
As $f$ is increased in integer steps from $0$ the value of $e^{-\pi j f}$ toggles between $1$ and $-1$. This is because $e^{j \pi} = -1$ and is periodic with $2\pi$.
...
1. **Write, compile, run and de-bug C++ programs using standard programming techniques** - My project clearly demonstrates my ability to write C++ programs consisting of multiple different independent modules. I have made heavy use of C++ classes in order to separate driver code for microcontroller peripherals from my main application.I have tried, where possible, to write modules in a generic way that increasing the possibility of code reuse on other platforms.

    I have demonstrated my knowledge of how to use the compiler and the linker in order to compile and combine source code into the write format for the target platform. I have also shown how to automate aspects of the compilation and linking process using a makefile.

    Early on in my project I attempted to get the GNU debugger, GDB, to work with my microcontroller but did not have any success, having spent quite a long time on the problem. Instead most of my debugging was done by printing information over a serial connection to the computer once I had written a driver for the UART.

2. **Implement a range of embedded software techniques on a microcontroller** - The constraints of writing software to run directly on an embedded microprocessor 
To succeed with my project I have had to use 
